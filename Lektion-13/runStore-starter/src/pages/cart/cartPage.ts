...
const content = document.querySelector('#shopping-cart') as HTMLDivElement;
...
const handleIncrementCart = (e: Event) => {
  const id = (e.target as Node).parentElement?.getAttribute('id') as string;
  cart.incrementItem(id);
  loadCart();
};

const handleDecrementCart = (e: Event) => {
  const id = (e.target as Node).parentElement?.getAttribute('id') as string;
  cart.decrementItem(id);
  loadCart();
};

const handleRemoveItemFromCart = (e: Event) => {
  const id = (e.target as Node).parentElement?.getAttribute('id') as string;
  cart.removeItem(id);
  loadCart();
};
...
const displayCartContent = (items: ICartItem[]) => {
  ...
  buttons.forEach((button) => {
    if ((button.firstChild as HTMLElement).classList.contains('fa-plus')) {
      button.addEventListener('click', handleIncrementCart);
    } else if ((button.firstChild as HTMLElement).classList.contains('fa-minus')) {
      button.addEventListener('click', handleDecrementCart);
    } else if ((button.firstChild as HTMLElement).classList.contains('fa-trash-can')) {
      button.addEventListener('click', handleRemoveItemFromCart);
    }
  });
};

const createProductDisplay = (items: ICartItem[]) => {
  let html = '<section>';
...
const createOrderInfo = (items: ICartItem[]) => {
